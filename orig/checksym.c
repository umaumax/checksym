...
/* ¥·¥ó¥Ü¥ë¥Æ¡¼¥Ö¥ë¤òÁí¤Ê¤á¤·¤Æ¡¤¥·¥ó¥Ü¥ëÌ¾¤Î¥ê¥¹¥È¤òºîÀ® */
static int make_symbol_table(char *filename, rh_t rh, rw_t rw, Elf32_Ehdr *ehdr,
  ...
  for (i = 0; i < shsize; i += shentsize) {
    ...
    name->flags = 0;
    if (rh(&symp->st_shndx) == SHN_COMMON) name->flags |= NAME_FLAG_NOALLOC;
...
/* ºÆÇÛÃÖ¥Æ¡¼¥Ö¥ë¤òÁí¤Ê¤á¤·¤Æ¡¤¥·¥ó¥Ü¥ëÌ¾¤Î¥ê¥¹¥È¤òºîÀ® */
static int make_reltxt_table(char *filename, rh_t rh, rw_t rw, Elf32_Ehdr *ehdr,
                             Elf32_Shdr *reltxt, Elf32_Shdr *symtab,
                             Elf32_Shdr *strtab) {
  ...
  head = (char *)ehdr;
  if ((rw(&reltxt->sh_type) != SHT_REL) && (rw(&reltxt->sh_type) != SHT_RELA)) {
    fprintf(stderr, "Invalid type. (%d)\n", rw(&reltxt->sh_type));
    exit(1);
  }
...
static int readar(char *filename, char *head, int size) {
  ...
  for (offset = SARMAG; offset < size;) {
    ...
    } else if ((arhdr->ar_name[0] == '/') && (arhdr->ar_name[1] == '/')) {
